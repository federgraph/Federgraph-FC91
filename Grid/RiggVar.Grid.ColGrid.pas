...
type
  ...
  public
    ...
    function ColumnToColorDef(cp: TBaseColProp; aColor: TAlphaColor): TColor; virtual;
  ...
  TCellSelectEvent = procedure (Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean) of object;
  ...
  public
    ...
    procedure ShowData;
    procedure UpdateAll;
    procedure HeaderClicked(col: Integer);
    procedure InitDisplayOrder(col: Integer);
    procedure InitDisplayOrderDir(col: Integer; Descending: Boolean);
    ...
    procedure DashText(SL: TStrings; aCaption: string);
...
begin
  ...
  try
    ...
    begin
      IsNormalRow := Odd(ARow div HeatSize);
...
procedure TColGrid.HeaderClicked(col: Integer);
var
  cp: TBaseColProp;
begin
  cp := ColsActive[col];
  if Assigned(cp) and cp.Sortable then
    InitDisplayOrderDir(col, not cp.Descending);
end;
...
begin
  cp := ColsActive[col];
  if Assigned(cp) and cp.Sortable then
    InitDisplayOrderDir(col, cp.Descending);
end;

procedure TColGrid.InitDisplayOrderDir(col: Integer; Descending: Boolean);
var
...
procedure TColGrid.DashText(SL: TStrings; aCaption: string);
var
  c, r: Integer;
  s, sh: string;
  l, lh: string;
...
begin
  ...
  begin
    ...
    begin
      cp := Self.FColsActive.Items[c];
      if not Assigned(cp) then continue;
      if cp.CharWidth <> 0 then
        w := cp.CharWidth
      else
        w := cp.Width div 10;

      if (r = 0) and (GridModel.HeaderRowIndex = 0) then
      begin
        sh := cp.Caption;
        while sh.Length < w do
        begin
          if cp.FAlignment = taRightJustify then
            sh := fch + sh
          else
...
function TBaseColProp.GetCanEdit: Boolean;
begin
  result := Assigned(OnFinishEdit2) or Assigned(OnFinishEdit);
end;
...
procedure TBaseColProp.SetCharWidth(const Value: Integer);
begin
  FCharWidth := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      cr := TBaseRowCollectionItem(FItems.Last);
      self.RemoveItem(cr);
      cr.Free;
    end;
  end;
...
function TBaseRowCollectionItem.ColumnToColorDef(cp: TBaseColProp; aColor: TAlphaColor): TColor;
begin
  result := aColor;
end;
