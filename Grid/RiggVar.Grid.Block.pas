...
begin
  ...
  begin
    ...
    ColGrid.GridModel := TColGridModel.Create(Grid);
...
begin
  if Grid.FinishEditFlag or not Grid.EditorMode then
  Grid.ShowData;
end;

procedure TGridBlock.SelChanged(Sender: TObject);
begin
  if Assigned(OnIndexChanging) then
    OnIndexChanging(Sender, Grid.Row - 1);
end;
