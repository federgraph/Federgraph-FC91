...
//    iv, iw, jv, jw: single;
    ...
    property FactorEQ: Integer read FaktorEQ write FaktorEQ;
...
function TFederEquation.rsqrt(const ANumber: single): single;
var
  ResultAsInt: UInt32 absolute Result;
begin
  Result := ANumber;
  ResultAsInt := $5F3759DF - (ResultAsInt shr 1);
  Result := Result * (1.5 - (ANumber * 0.5 * Result * Result));
//  Result := Result * (1.5 - (ANumber * 0.5 * Result * Result));
end;

function TFederEquation.TakesAbsolute: Boolean;
begin
  case Plot of
     5, 7, 10: result := True;
     else
...
function TFederEquation.GetBasisPlot: Integer;
begin
  case Plot of
     5: result := 3; // Abs(u + v) : u + v
     7: result := 0; // v
     10: result := 9; // u
     else
      result := Plot;
...
var
  SavedPlot: Integer;
  SavedPlusCap: Boolean;
  SavedMinusCap: Boolean;
begin
  { ignore PlusCap and MinusCap AND use unbroken BasisPlot }
  SavedPlusCap := PlusCap;
  SavedMinusCap := MinusCap;
  SavedPlot := Plot;

  PlusCap := False;
  MinusCap := False;
  Plot := GetBasisPlot; // i.e. 9 instead of 10
  result := GetValue(x, y);

  PlusCap := SavedPlusCap;
  MinusCap := SavedMinusCap;
  Plot := SavedPlot;
end;

function TFederEquation.GetValueL(x, y: single): single;
var
  SavedPlusCap: Boolean;
  SavedMinusCap: Boolean;
begin
  { ignore PlusCap and MinusCap }
  SavedPlusCap := PlusCap;
  SavedMinusCap := MinusCap;
