...
type
  ...
  public
    ...
    procedure ExportTable(ML: TStrings); override;
    procedure CheckForDuplicates(ML: TStringList); override;
    function GetShortcutString(fa: TFederAction): string; override;
    function GetActionGroup(fa: TFederAction): Integer; override;
    function GetShortcutReport(ML: TStrings): string; override;
  end;
...
procedure TActionHelper.CheckForDuplicates(ML: TStringList);
var
  fa: TFederAction;
  s: string;
begin
  ...
  ML.Clear;
  ML.Sorted := False;
  ML.CaseSensitive := False;
  ML.Duplicates := TDuplicates.dupIgnore;
end;
